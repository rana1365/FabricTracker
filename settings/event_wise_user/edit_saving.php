...
$department_name= $_POST['event_id'];

mysqli_query($con,"BEGIN");
mysqli_query($con,"START TRANSACTION") or die(mysqli_error());

$select_sql_for_duplicacy="select * from `event_wise_user` where `user_id`='$location' and `event_id`='$department_name'";
...
//    $insert_sql_statement="insert into `event_wise_user` ( `user_id`,`event_id`) values ('$location','$department_name')";
    $insert_sql_statement="UPDATE `event_wise_user` SET `user_id`='$location',`event_id`='$department_name' WHERE `id`= '$id'";
